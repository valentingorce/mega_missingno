...
class TestMatrix(unittest.TestCase):
    def setUp(self):
        ...
        self.simple_df = self.spark.createDataFrame(pd.DataFrame((np.random.random((20, 10)) > 0.5), columns=range(0, 10)).replace(False, None))
        np.random.seed(42)
        self.large_df = self.spark.createDataFrame(pd.DataFrame((np.random.random((250, 60)) > 0.5)).replace(False, None))
