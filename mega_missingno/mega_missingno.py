...
import warnings
from pyspark.sql.functions import array, col, sum, when, concat
...
def matrix(
    ...
    sample_height = min(10000, height)
    ...
    z = array([col(c).isNotNull() for c in df.columns])
    
    # create a new DataFrame to hold the encoded values
    colors_df = df.select([when(col(c).isNull(), "not_full").otherwise("full").alias(c) for c in df.columns])

    # concatenate the columns to create the encoded value column
    colors_df = colors_df.withColumn("enc", 
                                     concat(*[when(col(c) == "full", "0")\
                                              .otherwise("1") for c in colors_df.columns]))
    counts = colors_df.groupBy("enc").count().orderBy(col("count").desc())
    if counts.count()>sample_height:
      counts = counts.iloc[:sample_height]
    # fractions = {row['enc']:row['count']*sample_height/height for row in counts.collect()} # find a better fraction
    fractions = {row['enc']:sample_height/height for row in counts.collect()}
    sample_df = colors_df.sampleBy("enc", fractions=fractions, seed=42).toPandas()
    sample_height = len(sample_df)
    g = sample_df['enc'].apply(lambda x: [[1,1,1] if ch=='1' else list(color) for ch in x]).values
    g = np.array(g.tolist()).reshape((sample_height, sample_width, 3))

    width = sample_width
    height = sample_height
    df = pd.DataFrame(columns=df.columns,data=np.array(df.collect())[list(sample_df.index)])
...
def bar(
    ...
    count = df.count()
    nullity_counts = df.select([(count-sum(df[c].isNull().cast('int'))).alias(c) for c in df.columns])
    nullity_counts.show()
    ...
    if orientation == 'bottom':
        n_df = nullity_counts.select(*[(col(c)/count).alias(c) for c in nullity_counts.columns])
        n_df.show()
        n_df.toPandas().plot.bar(**plot_args)
        # .plot.bar(**plot_args)
        # (nullity_counts / count).plot.bar(**plot_args)
    else:
        (nullity_counts / count).plot.barh(**plot_args)
    ...
    if labels or (labels is None and len(df.columns) <= 50):
        ...
        if not log:
            ...
            ax2.set_yticklabels([int(n * df.count()) for n in ax1.get_yticks()], fontsize=fontsize)
        else:
        ...
        ax3.set_xticklabels(
            np.array([np.array(row) for row in nullity_counts.collect()]), fontsize=fontsize, rotation=label_rotation, ha='left'
        )
    else:
        # Create the numerical ticks.
        ax2 = ax1.twinx()
        ...
        if not log:
            ...
            ax2.set_xticklabels([int(n * df.count()) for n in ax1.get_xticks()], fontsize=fontsize)
...
def heatmap(
    ...
    # non_null_counts = df.select(*[count(when(~col(c).isNull() & (col(c) != ""), c)).alias(c) for c in df.columns])

    # # Get the columns that have non-zero variance
    # non_zero_var_cols = [c[0] for c in non_null_counts.rdd.map(lambda x: (x[0],)).filter(lambda x: x[0] > 0).collect()]
    
    # Count the number of non-null values for each column
    non_null_counts = df.select([col(c).isNotNull().cast("int").alias(c) for c in df.columns])

    # Get the columns that have non-zero variance
    non_zero_var_cols = [c[0] for c in non_null_counts.agg(*[sum(col(c)).alias(c) for c in df.columns]).rdd.first().asDict().items() if c[1] > 0]


    print(non_zero_var_cols)
    # Select only the columns with non-zero variance
    df = df.select(*non_zero_var_cols)
    for c in df.columns:
        df = df.withColumn(c, when(df[c].isNull(), 0).otherwise(1))
    print(df.columns)
    ...
    corr_mat = Correlation.corr(df_assembled, "features").collect()[0][0]

    # Mask the correlation matrix
    mask = np.zeros_like(corr_mat.toArray())
    mask[np.triu_indices_from(mask)] = True
    corr_mat = corr_mat.toArray()
    corr_mat = pd.DataFrame(columns=list(df.columns),
                            index=list(df.columns),
                            data=corr_mat)
    # corr_mat_masked = np.ma.masked_where(mask, corr_mat)
